...
**Реализация вывода элеметов массива в зигзагообразном порядке:**  
![IMG_5674](https://github.com/user-attachments/assets/93785863-b53b-437f-967c-02cfd5554a18)

Выводим элементы по заданной схеме, начиная с элемента с индексами 0, 0.
Задаем две переменные n0 и m0, которые будут отвечать за индексы элементов массива, а также счетчик для удобства.
Можем сделать вывод, что, если индекс строчки чётный, мы движемся вправо, а если нечётный - влево.
1. Выводя элементы первой (нулевой строчки), мы доходим до последнего с индексом [0][m-1]. Далее мы должны сдвинуться на строчку вниз. Флажком для сдвига будет дошедшее до m значение счетчика counter и m0. Мы уменьшаем m0 на один, чтобы не выйти за границы массива, и увеличиваем n0 на 1. 
2. Похожим образом мы доходим до первого элемента второй (первой) строчки с индексом [1][0]. Далее мы должны сдвинуться на строчку вниз. Флажком для сдвига будет дошедшее до 0 значение счетчика counter. Мы увеличиваем m0 на один, чтобы не выйти за границы массива, и увеличиваем n0 на 1. 
Таким образом мы доходим до конца массива.
